import TypeProducts from "../models/TypeProduct";


let serviceCreateNewTypeProduct = async (name) => {
    try {
        // Kiểm tra xem thể loại sản phẩm đã tồn tại hay chưa
        let checkNameTypeProduct = await TypeProducts.findOne({
        ...
        if (checkNameTypeProduct) {
            return { success: false, message: 'Tên thể loại sản phẩm đã tồn tại!' };  // Trả về thông báo thất bại
        }
        ...
        return { success: true, message: 'Thêm thể loại sản phẩm thành công!', typeProduct: newTypeProduct };  // Trả về thông báo thành công và đối tượng mới
    } catch (error) {
...
let serviceGetAllTypeProduct = async () => {
    try {
        ...
        console.error('Error in serviceGetAllTypeProduct:', error);
        return [];
    }
};
